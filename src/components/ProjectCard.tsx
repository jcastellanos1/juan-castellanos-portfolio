import { useReveal } from '../hooks/useReveal';

interface ProjectCardProps {
    title: string;
    bullets: string[];
    tags: string[];
    link: string;
    imageUrl?: string;
}

export const ProjectCard = ({ title, bullets, tags, link, imageUrl }: ProjectCardProps) => {
    const { ref, className } = useReveal<HTMLAnchorElement>(0.1);

    return (
        <a
            href={link}
            target="_blank"
            rel="noopener noreferrer"
            ref={ref}
            className={`block group md:grid md:grid-cols-12 md:gap-8 py-10 border-b border-border/10 last:border-0 hover:opacity-100 transition-all duration-500 ${className}`}
        >
            {/* Image Section */}
            <div className="md:col-span-4 mb-6 md:mb-0 relative overflow-hidden rounded-lg aspect-video md:aspect-auto bg-muted/10 border border-border/10 group-hover:border-accent/20 transition-colors">
                {imageUrl ? (
                    <div className="w-full h-full flex items-center justify-center text-muted/20 text-xs uppercase tracking-widest font-mono">
                        {/* Placeholder for real image implementation */}
                        <span className="sr-only">Project Image for {title}</span>
                        <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-white/0 opacity-50" />
                        <span className='z-10'>Image Space</span>
                    </div>
                ) : (
                    <div className="w-full h-full bg-muted/5 animate-pulse" />
                )}
            </div>

            {/* Content Section */}
            <div className="md:col-span-8 flex flex-col justify-center">
                <div className="mb-4 group-hover:translate-x-2 transition-transform duration-300">
                    <h3 className="text-2xl font-bold text-text group-hover:text-accent transition-colors flex items-center gap-3">
                        {title}
                        <svg className="w-5 h-5 opacity-0 group-hover:opacity-100 transition-opacity transform -translate-x-2 group-hover:translate-x-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
                        </svg>
                    </h3>
                </div>

                <ul className="space-y-3 mb-6">
                    {bullets.map((bullet, index) => (
                        <li key={index} className="text-muted text-sm leading-relaxed flex items-start gap-2">
                            <span className="mt-1.5 w-1 h-1 rounded-full bg-accent/60 shrink-0"></span>
                            <span>{bullet}</span>
                        </li>
                    ))}
                </ul>

                <div className="flex flex-wrap gap-2">
                    {tags.map((tag, index) => (
                        <span
                            key={index}
                            className="px-2.5 py-1 text-xs font-medium text-accent bg-accent/10 rounded-full border border-accent/10"
                        >
                            {tag}
                        </span>
                    ))}
                </div>
            </div>
        </a>
    );
};
